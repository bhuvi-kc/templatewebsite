...
// Parse a colour string ("#rgb", "#rrggbb", "rgb(...)", "rgba(...)") to 0–1 RGB.
...
/**
 * Fluid trail background. Renders full-bleed behind whatever sits above it.
 * Listens on `window` for pointer events so it still tracks the cursor even
 * when another element (like a Three.js canvas) sits visually on top.
 */
export default function FluidTrail({
  ...
  useEffect(() => {
    ...
    function updatePointer(clientX, clientY) {
      ...
      // Ignore moves outside the canvas's own screen area.
    ...
    // Listen on window, not the canvas — this is what lets the trail track
    // the cursor even when the Lanyard's Three.js canvas sits on top of it.
    ...
    function splatAt(x, y, dx, dy, col) {
      const aspect = canvas.width / canvas.height
      const mr = Math.max(1, propsRef.current.mouseRadius)
      ...
      gl.uniform1f(gl.getUniformLocation(pSplat, "u_aspect"), aspect)
      gl.uniform2f(gl.getUniformLocation(pSplat, "u_point"), x, y)
      gl.uniform3f(gl.getUniformLocation(pSplat, "u_color"), col[0], col[1], col[2])
      gl.uniform1f(gl.getUniformLocation(pSplat, "u_radius"), radius)
    ...
    function applyPointerInput() {
      ...
      const c = parseColor(propsRef.current.color)
    ...
    function step(dt) {
      ...
      gl.uniform1f(gl.getUniformLocation(pAdvect, "u_dt"), dt)
      const dur = Math.max(0.1, propsRef.current.trailDuration)
      const dyeDiss = 3 / dur
  ...
  return (
    <canvas
      ...
      style={{
        ...
        height: "100%",
        touchAction: "none",
        pointerEvents: "none", // purely visual — never blocks clicks/drags above it
        ...style,
      }}
    />
  )
